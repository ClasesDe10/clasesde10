...
export const PLATFORM_CONFIG_DEFAULTS = Object.freeze({
  ...
  ai: {
    ...
    internalAssistantEnabled: true,
    internalAssistantScanLimit: 1000,
    internalAssistantLongChatMessages: 20,
    internalAssistantConflictKeywordThreshold: 2,
    internalAssistantStaleChatHours: 24,
    internalAssistantStaleIncidentHours: 24,
    internalAssistantIncidentSummaryEntries: 4,
    internalAssistantDocumentReviewHours: 24,
    internalAssistantProfileMinPercent: 85,
    internalAssistantPatternWindowDays: 30,
    internalAssistantPatternThreshold: 3,
    internalAssistantDailyBriefMinScore: 58,
    internalAssistantDailyBriefMaxItems: 8,
    internalAssistantAutoCreateTasks: true,
    internalAssistantAutoCreateOpsAlerts: true,
  },
  notifications: {
...
export const PLATFORM_CONFIG_SECTIONS = Object.freeze([
  ...
  },
  {
    ...
    fields: [
      ...
      field('ai.internalAssistantEnabled', 'Asistente interno automatico', 'boolean'),
      field('ai.internalAssistantScanLimit', 'Elementos escaneados por IA interna', 'number', { min: 10, max: 5000, step: 10 }),
      field('ai.internalAssistantLongChatMessages', 'Chat largo tras mensajes', 'number', { min: 5, max: 500, step: 1 }),
      field('ai.internalAssistantConflictKeywordThreshold', 'Senales de conflicto minimas', 'number', { min: 1, max: 20, step: 1 }),
      field('ai.internalAssistantStaleChatHours', 'Chat sin cierre tras (h)', 'number', { min: 1, max: 1440, step: 1 }),
      field('ai.internalAssistantStaleIncidentHours', 'Incidencia sin avance tras (h)', 'number', { min: 1, max: 1440, step: 1 }),
      field('ai.internalAssistantIncidentSummaryEntries', 'Historial minimo para resumen', 'number', { min: 2, max: 100, step: 1 }),
      field('ai.internalAssistantDocumentReviewHours', 'Documento pendiente tras (h)', 'number', { min: 1, max: 1440, step: 1 }),
      field('ai.internalAssistantProfileMinPercent', 'Perfil minimo sin ayuda (%)', 'number', { min: 1, max: 100, step: 1 }),
      field('ai.internalAssistantPatternWindowDays', 'Ventana patrones repetidos (dias)', 'number', { min: 1, max: 365, step: 1 }),
      field('ai.internalAssistantPatternThreshold', 'Umbral patron repetido', 'number', { min: 2, max: 100, step: 1 }),
      field('ai.internalAssistantDailyBriefMinScore', 'Score minimo resumen diario', 'number', { min: 1, max: 100, step: 1 }),
      field('ai.internalAssistantDailyBriefMaxItems', 'Maximo prioridades resumen diario', 'number', { min: 3, max: 50, step: 1 }),
      field('ai.internalAssistantAutoCreateTasks', 'Crear tareas desde IA interna', 'boolean'),
      field('ai.internalAssistantAutoCreateOpsAlerts', 'Crear alertas desde IA interna', 'boolean'),
    ],
  },
  {
    id: 'notifications',
  ...
  },
  {
    ...
    description: 'Acompanamiento no invasivo de familias y profesores despues del matching.',
    fields: [
      field('followup.enabled', 'Seguimiento activo', 'boolean'),
      field('followup.scanLimit', 'Relaciones escaneadas', 'number', { min: 10, max: 5000, step: 10 }),
      field('followup.scheduleNudgeHours', 'Avisar si falta primer horario tras (h)', 'number', { min: 1, max: 720, step: 1 }),
      field('followup.proposedScheduleNudgeHours', 'Avisar propuesta sin responder tras (h)', 'number', { min: 1, max: 720, step: 1 }),
      field('followup.firstClassPrepHours', 'Preparar primera clase cuando falten (h)', 'number', { min: 1, max: 168, step: 1 }),
      field('followup.firstClassCheckinHours', 'Check-in tras primera clase (h)', 'number', { min: 1, max: 720, step: 1 }),
      field('followup.confirmationNudgeHours', 'Confirmacion pendiente tras (h)', 'number', { min: 1, max: 168, step: 1 }),
      field('followup.activeSilenceDays', 'Relacion activa sin siguiente clase tras (dias)', 'number', { min: 1, max: 365, step: 1 }),
      field('followup.qualityCheckCompletedClasses', 'Clases antes de pedir valoracion', 'number', { min: 1, max: 50, step: 1 }),
      field('followup.qualityCheckCooldownDays', 'Cooldown valoraciones (dias)', 'number', { min: 1, max: 365, step: 1 }),
      field('followup.repeatedCancellationWindowDays', 'Ventana cancelaciones repetidas (dias)', 'number', { min: 1, max: 365, step: 1 }),
      field('followup.repeatedCancellationThreshold', 'Umbral cancelaciones repetidas', 'number', { min: 2, max: 50, step: 1 }),
      field('followup.teacherActivityDropDays', 'Escalar relacion sin actividad tras (dias)', 'number', { min: 1, max: 365, step: 1 }),
      field('followup.adminEscalationHours', 'Escalar al admin tras bloqueo (h)', 'number', { min: 1, max: 1440, step: 1 }),
  ...
  },
  {
    ...
    description: 'Ayudas anticipadas antes de que una duda, bloqueo o incoherencia se convierta en incidencia.',
    fields: [
      field('proactiveAssist.enabled', 'Asistencia proactiva activa', 'boolean'),
      field('proactiveAssist.scanLimit', 'Elementos escaneados', 'number', { min: 10, max: 5000, step: 10 }),
      field('proactiveAssist.onboardingNudgeHours', 'Ayudar onboarding tras (h)', 'number', { min: 1, max: 720, step: 1 }),
      field('proactiveAssist.profileNudgeMinCompletion', 'Perfil minimo antes de avisar (%)', 'number', { min: 1, max: 100, step: 1 }),
      field('proactiveAssist.profileNudgeCooldownHours', 'Cooldown perfil (h)', 'number', { min: 1, max: 1440, step: 1 }),
      field('proactiveAssist.missingAvailabilityHours', 'Avisar franjas faltantes tras (h)', 'number', { min: 1, max: 1440, step: 1 }),
      field('proactiveAssist.requestAvailabilityNudgeHours', 'Avisar solicitud sin franjas tras (h)', 'number', { min: 1, max: 1440, step: 1 }),
      field('proactiveAssist.upcomingClassReadinessHours', 'Revisar clases proximas sin importes (h)', 'number', { min: 1, max: 720, step: 1 }),
      field('proactiveAssist.teacherPayoutReadinessHours', 'Revisar Bizum antes de pagar tras (h)', 'number', { min: 1, max: 720, step: 1 }),
      field('proactiveAssist.unreadCriticalNotificationHours', 'Aviso prioritario sin leer tras (h)', 'number', { min: 1, max: 720, step: 1 }),
      field('proactiveAssist.lowSupplyRequestHours', 'Solicitud con poca oferta tras (h)', 'number', { min: 1, max: 1440, step: 1 }),
      field('proactiveAssist.lowSupplyMinCandidates', 'Candidatos minimos suficientes', 'number', { min: 1, max: 50, step: 1 }),
      field('proactiveAssist.lowSupplyMinScore', 'Score minimo candidato util', 'number', { min: 0, max: 100, step: 1 }),
      field('proactiveAssist.verifiedTeacherIdleDays', 'Profesor verificado sin alumnos tras (dias)', 'number', { min: 1, max: 365, step: 1 }),
      field('proactiveAssist.staleAdminTaskHours', 'Tarea admin atascada tras (h)', 'number', { min: 1, max: 1440, step: 1 }),
