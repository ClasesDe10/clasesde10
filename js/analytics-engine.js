...
export const ANALYTICS_EVENT_CATALOG = Object.freeze([
  ...
  'class.created',
  'class.updated',
  'class_lifecycle_transition',
  'class.payment_review_started',
  'class.cancelled',
  'payment.created',
  'payment.verified',
  'message.sent',
...
function isGenericAnalyticsPersonLabel(value) {
  const key = analyticsPersonKey(value);
  if (!key || GENERIC_ANALYTICS_PERSON_LABELS.has(key)) return true;
...
function analyticsPersonName(role, id = '', ...values) {
  for (const value of values) {
    ...
    if (candidate && !isGenericAnalyticsPersonLabel(candidate)) return candidate;
  }
