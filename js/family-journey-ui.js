...
function renderAction(action, primary = false) {
  ...
  return `<button type="button" class="${klass}" data-family-journey-action="${escapeHtml(action.id)}" data-section="${escapeHtml(action.section || '')}">
...
function renderChecklistItem(item = {}) {
  const action = item.done || !item.actionId
    ? ''
    : ` data-family-journey-action="${escapeHtml(item.actionId)}" role="button" tabindex="0"`;
...
export function renderFamilyJourneyPanel(state = {}) {
  ...
  return `<section class="family-journey-card" data-family-journey-stage="${escapeHtml(state.stage)}">
    <div class="family-journey-main">
      <div class="family-journey-eyebrow">Tu siguiente paso</div>
    ...
    <aside class="family-journey-side" aria-label="Progreso de onboarding familiar">
