...
ClasesDe10 debe converger en una sola fuente de verdad: Firebase. La web
publica debe seguir siendo estatica y rapida, Netlify debe servir como
CDN/deploy, y Google Sheets/Apps Script debe quedar como legado apagado.
Supabase sigue como puente para Auth, dashboards y operativa no migrada.
...
- Firebase: cliente web, Firestore en `eur3`, reglas e indices.
- CSS: 2 archivos.
...
| Firebase Firestore | Vivo | Leads publicos y profesores importados | Firebase |
| Google Sheets | Archivo | Historico exportado | Sheet externo |
| Apps Script | Apagado | Funciones no-op | `clasp-project/main.js` |
| Gemini en Apps Script | Apagado | Matching antiguo | Apps Script |
...
| Supabase y Sheets como datos operativos | La web ya no llama Apps Script | Firebase gana para nuevos flujos |
...
4. Inserta en Firestore `leadsPublicos` con reglas anonimas de solo creacion.
5. Admin revisa temporalmente en Firebase Console.
6. Pendiente ideal: migrar panel admin, automatizar scoring, asignacion y notificacion.
...
| `FIREBASE_PROJECT_ID` | Firebase CLI/config | Publica |
...
- Firebase project: `clasesde10-50add`.
- Firebase JS CDN: `https://www.gstatic.com/firebasejs/`.
- Firestore API: `https://firestore.googleapis.com`.
...
El riesgo no es tecnico puro. Es operativo: si Supabase, Firebase y Sheets
operan sin limites claros, hay tres sistemas de negocio. La decision CTO es:
Firebase para nuevos flujos, Supabase como puente, Sheets/Apps Script apagado y
solo historico.
