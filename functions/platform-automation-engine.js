...
const DEFAULT_AUTOMATION_RULES = [
  ...
  },
  {
    ...
    actions: [
      ...
      { type: 'notification', target: { userUid: { path: 'computed.familyUid' }, role: 'familia' }, title: 'Confirma si la clase se dio', body: 'Han pasado 24h desde la {{computed.classLabel}} y necesitamos confirmar si se realizo.', payload: { classId: { path: 'computed.id' }, url: '/pages/login.html' }, options: { type: 'class_unmarked_after_24h', priority: 'high', key: 'family' } },
      ...
      { type: 'patch', collection: 'clases', docId: { path: 'computed.id' }, data: { trustPenaltyEvents: { class_unmarked_family: { type: 'notification_responsibility_penalty', notificationType: 'class_unmarked_after_24h', appliedToRole: 'familia', role: 'familia', appliedToUid: { path: 'computed.familyUid' }, userUid: { path: 'computed.familyUid' }, points: -2, reason: 'No confirmo la asistencia de la clase dentro de las 24h posteriores.', source: 'platform_automation' } } }, options: { key: 'trust_family' } },
