...
const DEFAULT_AUTOMATION_RULES = [
  ...
  },
  {
    ...
    actions: [
      ...
      { type: 'notification', target: { userUid: { path: 'computed.teacherUid' }, role: 'profesor' }, title: 'Nueva asignacion', body: 'Tienes un nuevo alumno de {{computed.subject}}. Revisa la asignacion en Mis alumnos.', payload: { requestId: { firstOf: ['data.requestId', 'data.solicitud_id'] }, assignmentId: { path: 'computed.id' }, url: '/pages/dashboard/profesor.html#alumnos' }, options: { type: 'assignment_created', priority: 'high', key: 'teacher' } },
      { type: 'notification', target: { userUid: { path: 'computed.familyUid' }, role: 'familia' }, title: 'Profesor asignado', body: 'Ya tienes profesor para {{computed.subject}}. Entra en Mis profesores y propón el horario semanal.', payload: { requestId: { firstOf: ['data.requestId', 'data.solicitud_id'] }, assignmentId: { path: 'computed.id' }, url: '/pages/dashboard/familia.html#profesores' }, options: { type: 'assignment_created', priority: 'high', key: 'family' } },
  ...
  },
  {
    ...
    actions: [
      ...
      { type: 'notification', target: { userUid: { path: 'computed.teacherUid' }, role: 'profesor' }, title: 'Clase pendiente de marcar', body: 'Han pasado 24h desde la {{computed.classLabel}} y sigue sin marcarse si se dio o no.', payload: { classId: { path: 'computed.id' }, url: '/pages/login.html' }, options: { type: 'class_unmarked_after_24h', priority: 'high', key: 'teacher' } },
      { type: 'notification', target: { targetRole: 'admin', role: 'admin' }, title: 'Clase sin cerrar', body: 'Han pasado 24h y la {{computed.classLabel}} sigue pendiente de confirmacion.', payload: { classId: { path: 'computed.id' }, url: '/pages/login.html' }, options: { type: 'class_unmarked_after_24h', priority: 'high', key: 'admin' } },
      { type: 'patch', collection: 'clases', docId: { path: 'computed.id' }, data: { trustPenaltyEvents: { class_unmarked_teacher: { type: 'notification_responsibility_penalty', notificationType: 'class_unmarked_after_24h', appliedToRole: 'profesor', role: 'profesor', appliedToUid: { path: 'computed.teacherUid' }, userUid: { path: 'computed.teacherUid' }, points: -2, reason: 'No marco si la clase se dio o no dentro de las 24h posteriores.', source: 'platform_automation' } } }, options: { key: 'trust_teacher' } },
  ...
  },
  {
    ...
    actions: [
      ...
      { type: 'notification', target: { userUid: { path: 'computed.familyUid' }, role: 'familia' }, title: 'Justificante vencido', body: 'Ha pasado el margen de 48h para subir el justificante de {{computed.paymentAmount}}.', payload: { paymentId: { path: 'computed.id' }, url: '/pages/login.html' }, options: { type: 'payment_overdue', priority: 'critical', key: 'family' } },
      { type: 'notification', target: { targetRole: 'admin', role: 'admin' }, title: 'Justificante vencido', body: 'Revisar justificante vencido por {{computed.paymentAmount}}.', payload: { paymentId: { path: 'computed.id' }, url: '/pages/login.html' }, options: { type: 'payment_overdue', priority: 'critical', key: 'admin' } },
      ...
      { type: 'patch', collection: 'pagos', docId: { path: 'computed.id' }, data: { trustPenaltyEvents: { payment_overdue_family: { type: 'notification_responsibility_penalty', notificationType: 'payment_overdue', classId: { firstOf: ['data.classId', 'data.clase_id'] }, appliedToRole: 'familia', role: 'familia', appliedToUid: { path: 'computed.familyUid' }, userUid: { path: 'computed.familyUid' }, points: -2, reason: 'No subio el justificante dentro de las 48h posteriores al dia acordado de pago.', source: 'platform_automation' } } }, options: { key: 'trust_family' } },
  ...
  },
  {
    ...
    priority: 171,
    actions: [
      { type: 'automationEvent', summary: 'Horario propuesto desde el panel de clases.', severity: 'info' },
      { type: 'notification', target: { userUid: { firstOf: ['data.recipientUid', 'data.toUid'] }, role: { firstOf: ['data.recipientRole', 'data.role'] } }, title: 'Horario propuesto', body: '{{computed.scheduleLabel}}. Tienes una respuesta pendiente en tu panel.', payload: { chatId: { path: 'data.chatId' }, assignmentId: { path: 'data.chatId' }, proposalId: { path: 'computed.id' }, url: '/pages/login.html' }, options: { type: 'schedule_proposed', priority: 'high', key: 'recipient' } },
      { type: 'crmTask', title: 'Horario pendiente de respuesta', description: 'Hay una propuesta de clase esperando aceptacion o alternativa.', options: { priority: 'normal', tags: ['horario', 'calendario'], dueAfterMinutes: configNumber('automation.scheduleProposalFollowUpMinutes', 1440) } },
  ...
  },
  {
    ...
    actions: [
      ...
      { type: 'notification', target: { userUid: { firstOf: ['data.proposedByUid', 'data.createdByUid'] }, role: { firstOf: ['data.proposedByRole', 'data.role'] } }, title: 'Horario rechazado', body: '{{computed.scheduleLabel}} se ha rechazado. Abre tu panel y propón otra alternativa.', payload: { chatId: { path: 'data.chatId' }, assignmentId: { path: 'data.chatId' }, proposalId: { path: 'computed.id' }, url: '/pages/login.html' }, options: { type: 'schedule_rejected', priority: 'high', key: 'proposer' } },
      { type: 'crmTask', title: 'Recoordinar horario', description: 'Una propuesta de horario ha sido rechazada. Si no hay nueva propuesta, intervenir.', options: { priority: 'normal', tags: ['horario', 'calendario'], dueAfterMinutes: configNumber('automation.rejectedScheduleFollowUpMinutes', 1440) } },
