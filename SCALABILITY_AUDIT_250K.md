...
La plataforma ya esta sobre Firebase, con Hosting, Firestore, Auth, Cloud Functions, reglas, PWA, notificaciones, matching, pagos, calendario, reputacion y automatizaciones. La base es valida para crecer, pero antes de esta fase existian cuatro riesgos serios de escala:
...
Implementado en codigo pero no desplegado por bloqueo externo:

- Cloud Functions `processSystemJobs`.
- Cloud Functions `rollupScaleMetrics`.
- Cambio de trigger para encolar `matching.request`.

Error exacto obtenido al intentar desplegar Functions:

```text
Your project clasesde10-50add must be on the Blaze (pay-as-you-go) plan to complete this command. Required API artifactregistry.googleapis.com can't be enabled until the upgrade is complete.
```

Conclusion: la arquitectura y el codigo estan preparados y verificados, pero la ejecucion automatica server-side de la cola requiere activar Blaze para permitir Cloud Functions v2.
...
- `processSystemJobs`: se ejecuta cada 5 minutos, reclama trabajos por lease, procesa en lotes y aplica reintentos con backoff exponencial.
...
- `rollupScaleMetrics`: cada 15 minutos genera un snapshot operativo.
...
- Matching en Functions usa limites configurables:
...
### 8. Cache corta en Cloud Functions
...
Riesgo anterior: triggers con trabajo pesado directo.
...
- Algunas Functions siguen haciendo fan-out directo de notificaciones. El siguiente paso es encolar fan-out masivo.
