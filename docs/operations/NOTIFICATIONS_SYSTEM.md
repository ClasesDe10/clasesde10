...
Firestore `notificaciones` es la fuente de verdad. El chat, las automatizaciones del worker y el panel admin escriben ahi los eventos importantes. La UI solo lee, marca como leido y registra tokens push del dispositivo.
...
1. Un evento ocurre: mensaje, solicitud, justificante de pago, clase sin marcar, documento, incidencia, perfil actualizado.
2. Codigo de confianza escribe un documento en `notificaciones`.
3. La app lo muestra en la pestana Chat / Notificaciones.
4. Si el documento incluye canal `push`, el worker de GitHub Actions busca tokens activos y envia FCM en el siguiente barrido.
...
- workflow `Firebase automation worker without Blaze` activo con credenciales Firebase Admin.
...
- Las notificaciones automaticas se crean desde el worker autorizado.
...
- Justificantes familiares pendientes o vencidos; los vencidos tras 48h registran penalizacion de reputacion.
