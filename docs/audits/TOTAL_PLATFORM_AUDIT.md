...
ClasesDe10 ya no es solo una landing: tiene Firebase Hosting en produccion, Firebase Auth operativo, Firestore con datos reales, automatizacion gratuita por GitHub Actions y formularios publicos guardando leads. La base es recuperable y puede convertirse en una plataforma seria sin Firebase Blaze.

El problema principal no es que falte "una feature". El problema es que el producto esta partido en dos mundos:

- Firebase sostiene hosting, auth, datos operativos, leads, usuarios, profesores, solicitudes y automatizacion por worker.
- Supabase queda como forma de API legacy compatible, no como backend real de produccion.
...
8. Chat/documentos/pagos/notificaciones se mantienen sobre Firebase.
9. La capa legacy `db.from()` se sustituye gradualmente por adaptadores Firebase por dominio.
...
| `npm run check:automation` | OK. Worker y motores compartidos sin errores de sintaxis. |
...
| `npm run audit:supabase` | 0 dependencias Supabase reales; quedan llamadas `db.from()` de compatibilidad Firebase. |
...
| Matching lee aliases importados | Completado | El worker y `js/ai-engine.js` reconocen `subjects`, `levels`, `hourlyRate` y `tarifaHora`. |
| Automatizar matching sin Blaze | Resuelto con worker gratuito | `scripts/firebase-automation-worker.mjs` corre desde GitHub Actions cada hora; Cloud Functions no son parte de produccion. |
...
- Firestore Rules validan campos criticos y roles.
- Firestore Indexes cubren consultas operativas principales.
- `scripts/firebase-automation-worker.mjs` procesa leads, matching, clases,
  pagos, justificantes, incidencias, notificaciones, push, metricas y jobs.
- Esta es la base de automatizacion gratuita: reglas deterministas primero, LLM
  opcional despues.

### API legacy compatible

No hay cliente Supabase real en runtime. Algunos archivos conservan llamadas
`db.from()` para evitar una migracion masiva de UI; esas llamadas pasan por
