...
for (const [name, html] of [
  ...
  assert.match(html, /dashboard\.css\?v=\d{8}-[a-z0-9-]+/, `${name} dashboard must bust CSS cache.`);
}
...
assert.match(familyDashboard, /Día de pago/);
assert.match(familyDashboard, /id="plan-pago-contexto"/);
assert.match(familyDashboard, /Guardar día de pago/);
assert.match(familyDashboard, /id="plan-pago-frecuencia"/);
assert.match(familyDashboard, /Cada 15 dias/);
assert.match(familyDashboard, /Avisar que no se dio/);
assert.match(familyDashboard, /Confirmar que se dio/);
assert.match(familyDashboard, /id="stat-pagos-pendientes"/);
...
assert.match(teacherDashboard, /Guardar resultado/);
...
assert.match(teacherDashboard, /teacherClassDetailBody[\s\S]*formatEuros\(amount\)/);
